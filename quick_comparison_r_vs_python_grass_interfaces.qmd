...
In previous tutorials we have gone through some of the basics of working 
with GRASS GIS through R and Python. As you might recall then, there's
an R package called [rgrass]() that provides basic functionality to read and write
data from and into GRASS database as well as to execute GRASS tools in either
existing or temporary GRASS projects. The GRASS Python API on the other hand
is composed of various packages including classes and functions for low and high
level tasks. In a previous tutorial we explored the basic functionality of 
...
1. **Load the library**: Either if we are working in R or in Python, we need to load the
libraries that will, in this case, allow us to interface with GRASS GIS 
functionality and data. For the Python case, we first need to add the GRASS
python path to our system's path.
...
2. **Start GRASS GIS session**: Once we loaded or imported the packages, we start a 
GRASS session. In both cases we need to pass the path to GRASS database, 
a project and a mapset name. In the case of R, we are also required to pass
the path to GRASS binaries, that in the Python case was needed for library 
import above. Here, it is worth noting that while grass.scrip and grass.jupyter
init functions take the same arguments, `gj.init` also sets other environmental 
variables to streamline work within Jupyter Notebooks, eg., overwrite is set to
true.
...
and flags. While in R we basically use `execGRASS()`, in the Python case, we have 
different wrappers for GRASS commands depending on the nature of their output.
...
4. **Read raster and vector data into other R or Python formats**: As explained
in previous tutorials, *rgrass* functions `read_RAST()` and `read_VECT()` convert
GRASS raster and vector maps into terra's SpatRaster and SpatVector objects 
within R. In the case of Python, we only have such a functionality for GRASS 
raster maps that can be read converted into numpy arrays through 
`gs.array.array()`.
...
workflow, is better to clean up explicitely.
