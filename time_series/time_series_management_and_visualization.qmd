...
sys.path.append(
    subprocess.check_output(["/home/akratoc/dev/grass/grass_main/bin.x86_64-pc-linux-gnu/grass", "--config", "python_path"], text=True).strip()
)
...
```{python}
path_to_project = "/data_8TB/3564515/grassdata_eu_laea_northern_italy_LST_1km_daily_celsius_reconstructed_ZLIB/eu_laea/italy_LST_daily"
...
There are different ways of registering maps. We can either pass a sorted list of maps or a text file with one map per line and optionally start 
and end dates, and semantic labels. It is also important to understand if our 
data represent time intervals (e.g. precipitation over a period of time) or time instances (events), and in the case of intervals, 
if they are regular (e.g., monthly) or irregular, as all these will determine different 
...
<!-- https://training.gismentors.eu/grass-gis-irsae-winter-course-2018/_static/scripts/sentinel-timestamp.py -->
...
With the Python Pandas package we can simple read in the output of t.rast.list as a DataFrame:
...
same filtering can be applied
in tools to determine maps that will be processed.
